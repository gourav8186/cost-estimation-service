...
$selCount = "SELECT COUNT(*) AS total FROM `ces_series`";
...
<section class="boxShadow p-3">
    <div class="headLine border-bottom border-2 d-flex justify-content-between py-2">
        <h5 class="font18 boldText">Series</h5>
        <a href="<?= ADD_SERIES; ?>" class="btn subBtn py-1">Add New</a>
    </div>
        ...
        <div class="tableResponsive py-3">
            <table class="table table-bordered text-center align-middle">
                ...
                <tbody>
                    ...
                    $getData = "SELECT * 
                FROM `ces_series` 
                INNER JOIN `ces_mobile_company` ON ces_series.s_company = ces_mobile_company.mbc_id 
                INNER JOIN `ces_model` ON ces_series.s_model = ces_model.m_id LIMIT $start,$limit";
                    ...
                    if (mysqli_num_rows($getExe) > 0) {
                        ...
                        while ($fetch = mysqli_fetch_array($getExe)) {
                            ...
                            $seriesCompany = $fetch['mbc_name'];
                            $seriesModel = $fetch['m_model'];
                            $seriesName = $fetch['s_seriesname'];
                            $status = $fetch['s_status'];
                            $updateLink = ADD_SERIES . '?update=' . $seriesId;
                    ?>
                            <tr>
                                <th> <input type="checkbox" name="items[]" value="<?= $seriesId; ?>"></th>
                                <th scope="row"><?= $i; ?></th>
                                <td><?= $seriesCompany; ?></td>
                                <td><?= $seriesModel; ?></td>
                                <td><?= $seriesName; ?></td>
                                <td>
                                    <ul class="actionBox ps-0 mb-0">
                                        ...
                                        <li><a title="Delete" class="deleteBtn cursor" data-element-id="<?= $seriesId; ?>"><i class="fa-solid fa-trash-can"></i> Delete</a></li>
                                        <li><select name="" id="" class="form-control py-1" onchange="chkStatus(this, '<?= $seriesId; ?>')">
