...
$selCount = "SELECT COUNT(*) AS total FROM `ces_mobile_company`";
...
<section class="boxShadow p-3">
    <div class="headLine border-bottom border-2 d-flex justify-content-between py-2">
        <h5 class="font18 boldText">Mobile Companies</h5>
        <a href="<?= ADD_MOBILE_COMPANY; ?>" class="btn subBtn py-1">Add New</a>
    </div>
    <form id="tableDataList">
        <div class="tableResponsive my-3">
            <table class="table table-bordered text-center align-middle">
                ...
                <tbody>
                    <?php
                    $getData = "SELECT * FROM `ces_mobile_company` LIMIT $start,$limit";
                    ...
                    if (mysqli_num_rows($getExe) > 0) {
                        ...
                        while ($fetch = mysqli_fetch_array($getExe)) {
                            ...
                            $status = $fetch['mbc_status'];
                            $updateLink = ADD_MOBILE_COMPANY . '?update=' . $mbcId;
                    ?>
                            <tr>
                                <th> <input type="checkbox" name="items[]" value="<?= $mbcId; ?>"></th>
                                <th scope="row"><?= $i; ?></th>
                                <td class="imgCol"><img src="../upload-images/<?php echo $mbcImg; ?>" alt=""></td>
                                <td><?= $mbcName; ?></td>
                                <td>
                                    <ul class="actionBox mb-0 ps-0">
                                        <li><a href="<?= $updateLink; ?>" class="cursor editBtn" title="Edit"><i class="fa-regular fa-pen-to-square"></i> Edit</a></li>
                                        <li><a title="Delete" class="deleteBtn cursor" data-element-id="<?= $mbcId; ?>"><i class="fa-solid fa-trash-can"></i> Delete</a></li>
                                        <li><select name="" id="" class="form-control py-1" onchange="chkStatus(this, '<?= $mbcId; ?>')">
